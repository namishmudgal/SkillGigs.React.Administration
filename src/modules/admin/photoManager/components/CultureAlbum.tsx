...
class CultureAlbum extends React.Component<Props, any> {
    ...
    return (
			<Card fluid className="rdus-none right pad-equal0">
				<Card.Content>
					...
					Company Culture Album
					</Header>
					<p className="fnt08">Showcase your company's culture by adding pictures.</p>
					<p className="fnt08">Upload as many as you wish and you an arrange the order by drag and drop.</p>
					<div style={{ position: 'relative' }}>
						<Button size="large" className="main-primary submit rdus-none mrg-tp04 wfull-mb">
							<Icon name="upload" /> ADD NEW PICTURE(S)
						</Button>
						<input name="img" type="file" accept="image/*" id="culture-image" className="hidden-upload" onChange={this.onImageUpload} />
					</div>
					{
					...
					<div className="actions mrg-tp04 bdr-tp">
						...
						<Button.Group floated='right' className="mrg-tp02">
							...
							<Button positive className="btn-pink txt-wht bdr-rdus-rmv" onClick={() => callbackFromParent(2)}>Next</Button>
						</Button.Group>
					</div>
				</Card.Content>
			</Card>
...
export default (withLocalize(CultureAlbum) as any);